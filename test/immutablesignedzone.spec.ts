...
describe("ImmutableSignedZone", function () {
  ...
  describe("Signer management", async () => {
    ...
    it("cannot add deactivated signer", async () => {
      ...
      ).to.be.revertedWithCustomError(contract, "SignerCannotBeReauthorized");
    });
    ...
    it("already active signer cannot be added", async () => {
      ...
      await expect(
        contract.addSigner(users[1].address)
      ).to.be.revertedWithCustomError(contract, "SignerAlreadyActive");
    });
  });
  ...
  describe("Order Validation", async function () {
    ...
    it("validateOrder reverts with non 0 SIP6 version", async function () {
      ...
      await expect(
        contract.validateOrder(mockZoneParameter(extraData))
      ).to.be.revertedWithCustomError(contract, "UnsupportedExtraDataVersion");
    });
    ...
    it("validateOrder reverts with wrong consideration", async function () {
      ...
      ).to.be.revertedWithCustomError(contract, "SubstandardViolation");
    });
    ...
    it("validateOrder reverts a valid order after expiration time passes ", async function () {
      ...
      await advanceBlockBySeconds(90);

      expect(
        await contract.validateOrder(
          mockZoneParameter(extraData, consideration)
        )
      ).to.be.revertedWithCustomError(contract, "SignatureExpired"); // ZoneInterface.validateOrder.selector
    });
    ...
    it("validateOrder reverts incorrectly signed signature with context", async function () {
      ...
      expect(
        contract.validateOrder(mockZoneParameter(extraData, consideration))
      ).to.be.revertedWithCustomError(contract, "SignerNotActive");
    });
  });
});
